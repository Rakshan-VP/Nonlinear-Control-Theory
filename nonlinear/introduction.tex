...
\captionof{figure}{Spring–mass–damper system with external force $F(t)$.}
\label{fig:spring}
\end{example}
...
\captionof{figure}{Phase portrait}
\label{fig:phase}
\end{example}
...
Trajectories form closed orbits, indicating a neutrally stable center (Fig.~\ref{fig:lti_phase}c).
    ...
    \captionof{figure}{Phase portraits of three different LTI systems: 
    (a) saddle point, 
...
\begin{example}[Vector Field of Van der Pol Oscillator: Stable vs. Unstable]
    ...
    The corresponding vector fields for $\mu=1$ are shown below.
        ...
        \captionof{figure}{Phase portraits of the Van der Pol oscillator for $\mu=1$: stable limit cycle (left) and unstable limit cycle (right).}
\end{example}
